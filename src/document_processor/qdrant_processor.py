...
class DocProcessor(BaseClass):
    ...
    def __init__(self) -> None:
        ...
        self.get_files_processed_failed()
        ...
        if self.use_page_content_similarity_useit:
            self.qdrant_obj = QdrantDocRetriever(self.vector_db_path,
                                                self.use_page_content_similarity_cname,
                                                self.use_vector_embedding_model,
                                                self.use_lower_embedding_size)
        elif self.use_topic_similarity_useit:
            self.qdrant_topics_obj = QdrantDocRetriever(self.vector_db_path,
                                                        self.use_topic_similarity_cname,
                                                        self.use_vector_embedding_model,
                                                        self.use_lower_embedding_size)
        elif self.use_both_useit:
            self.qdrant_obj = QdrantDocRetriever(self.vector_db_path,
                                                self.use_both_content_cname,
                                                self.use_vector_embedding_model,
                                                self.use_lower_embedding_size)
            self.qdrant_topics_obj = QdrantDocRetriever(self.vector_db_path,
                                                        self.use_both_topic_cname,
                                                        self.use_vector_embedding_model,
                                                        self.use_lower_embedding_size)
    ...
    def get_files_processed_failed(self,):
    ...
    def process_data(self,files=None):
        ...
        for each_file in processed_files:
            _,tail = os.path.split(each_file)
            processed_file_names.append(tail)
        
        start_time_total = time.time()
        for each_file_path in files:
            _,each_file_path_name = os.path.split(each_file_path)
            if each_file_path_name not in processed_file_names:
                try:
                    ...
                    start_time = time.time()
                    # upload documents
                    self.qdrant_obj.upload_data_records(documents=document_list)
                ...
                self.logger.info(f"{each_file_path} already processed.")
    ...
    def get_similar_docs_with_filter(self,text,filter='',limit=5):
        match_found = self.qdrant_obj.get_records_with_filter_dict(text,filter,limit)
